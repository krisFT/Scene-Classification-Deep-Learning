...
class ImageLabelingLogger(tf.keras.callbacks.Callback):
    ...
    def __init__(self, datasets, log_dir="logs/image_labels"):
        super(ImageLabelingLogger, self).__init__()

        self.datasets = datasets
        self.task = datasets.task
        self.log_dir = os.path.join(log_dir, "image_labels")
    ...
    def log_image_labels(self, epoch_num, logs):
        ...
        file_writer_il = tf.summary.create_file_writer(self.log_dir)
...
class ConfusionMatrixLogger(tf.keras.callbacks.Callback):
    ...
    def log_confusion_matrix(self, epoch, logs):
        ...
        test_pred = []
        test_true = []
        for i in range(len(self.datasets.test_data) - 1):
            data = self.datasets.test_data[i]
            test_pred.append(self.model.predict(data[0]))
            test_true.append(data[1])
